...
|---------|-----------|-------|
| **Workspace** | `<workspace-root>/.agent/intel/` | Chỉ có trong một dự án cụ thể |
...
```bash
mkdir -p ~/.gemini/antigravity/intel/code-review
```
...
```bash
mkdir -p .agent/intel/license-header-adder/resources
```

### Bước 2: Tạo file template

**`.agent/intel/license-header-adder/resources/HEADER.txt`**:
...
**`.agent/intel/license-header-adder/SKILL.md`**:
